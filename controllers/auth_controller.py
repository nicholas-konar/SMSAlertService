...
def send_to_create():
    ...
    try:
        ...
        app.logger.info(
            f'Sending OTP to prospective user at {ph}. Resends = {session["otp_resends"]}.'
        )
        ...
        app.logger.info(
            f"OTP delivery error: TwilioRestException for phone number {ph}"
        )
...
def resend_to_create():
    ...
    try:
        ...
        session["otp"] = otp_hash
        return jsonify(
            {"Status": SUCCESS, "FlowType": flow_type, "Message": RESEND_MSG}
        )
...
def validate_to_create():
    ...
    if session.get("otp_attempts") >= MAX_ATTEMPTS:
        app.logger.info(
            f"Max attempts limit (in session only) reached by prospective user. Denied resend."
        )
        return jsonify({"Status": BLOCKED, "Message": BLOCKED_MSG})
    ...
    if authenticated:
        ...
        app.logger.error(
            f'Prospective user failed authentication {session.get("otp_attempts")} time(s).'
        )
...
def send_to_recover():
    ...
    try:
        ...
        app.logger.debug(
            f'Sending OTP to {user.username}. Resends = {session["otp_resends"]}.'
        )
...
def resend_to_recover():
    ...
    if user.blocked:
        app.logger.info(
            f"Denied OTP resend to user {user.username}: Blocked status is {user.blocked}."
        )
        return jsonify({"Status": BLOCKED, "Message": BLOCKED_MSG})
    ...
    try:
        otp_hash = AlertService.send_otp(user.phonenumber)
        app.logger.debug(
            f'Resending OTP to {user.username}. Resends = {session["otp_resends"]}.'
        )
        session["otp_resends"] += 1
        session["otp"] = otp_hash
        return jsonify(
...
def validate_to_recover():
    ...
    if authenticated:
        ...
        session["otp_attempts"] += 1
        app.logger.error(
            f'{user.username} failed authentication {session.get("otp_attempts")} time(s).'
        )
        return jsonify({"Status": FAIL, "Message": RETRY_MSG})
