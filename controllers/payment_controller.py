...
def paypal_webhook():
    ...
    if webhook["event_type"] == "PAYMENT.CAPTURE.COMPLETED":
        ...
        if not authenticated:
            app.logger.error(
                f"Phony order #{order_id} found in webhook. Full webhook request: {webhook}"
            )
            return make_response("Forbidden", 403)

        if found := DAO.get_user_by_order_id(order_id):
            app.logger.error(
                f"Order #{order_id} already fulfilled under user_id {found.id}. Requested by user_id {user_id}."
            )
            ...
            # Payment data
            seller_receivable_breakdown = purchase_units["payments"]["captures"][0][
            ...
            if success:
                AlertService.send_order_confirmation(
                    user=user, order_description=order_description
                )
                AlertService.send_admin(
                    f"{user.username} purchased {order_description} for {gross}."
                )

            else:
                AlertService.send_admin(
                    f"Failed to fill order #{order_id} for {user.username} in database."
                )
