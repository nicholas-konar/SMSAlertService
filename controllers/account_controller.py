...
def login():
    ...
    if user is None:
        ...
        resp.set_cookie(
            "sms_alert_service_login", value=token, secure=True, httponly=True
        )
        ...
        app.logger.error(
            f'Incorrect password entered by {user.username}. Login attempts = {session.get("login_attempts")}'
        )
        return jsonify({"Status": FAIL, "Message": INVALID_LOGIN_MSG})
...
def edit_info():
    ...
    user = DAO.get_user_by_id(user_id)
    return render_template(
        "edit-info.html", username=user.username, current_phone=user.phonenumber
    )
...
def reset_password():
    if session.get("authenticated"):
        ...
        return (
            jsonify({"Status": SUCCESS, "Message": PW_RESET_SUCCESS_MSG})
            if success
...
def recover_account():
    ...
    return (
        render_template("reset-password.html")
        if authenticated
