import twilio, app
from dao import DAO
from resources.sms_templates import (
...
class AlertService:
    ...
    def send_alerts(alerts):
        for alert in alerts:
            body = STANDARD_ALERT_MSG.format(
                subreddit=alert.subreddit, keywords=alert.keywords, url=alert.url
            )
            twilio_object = twilio.send_message(body=body, ph=alert.owner.phonenumber)
            app.logger.info(
                f"Sent alert to {alert.owner.username} with SID {twilio_object.sid}"
            )
    ...
    def send_order_confirmation(user, order_description):
        ...
        app.logger.info(
            f"Notified {user.username} that their order was filled. SID: {message.sid}"
        )
